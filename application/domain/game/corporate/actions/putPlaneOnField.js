(function ({ joinPortId, targetPortId, targetPortDirect, joinPortDirect }, initPlayer) {
  ...
  if (targetGame.gameConfig === 'competition' && joinPlane.eventData.integrationPlane) {
    if (targetPlane.anchorGameId) {
      // !!!! доделать и проверить
      targetGame.toggleEventHandlers(handler, { targetId }, initPlayer);
      throw new Error('Этот блок уже был использован другой командой');
    } else {
      // нужно установить anchorGameId для последующего initGameFieldsMerge
      targetPlane.set({ anchorGameId: initPlayer.gameId });
    }
  }
