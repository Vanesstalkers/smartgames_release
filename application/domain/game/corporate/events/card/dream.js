(function event() {
  ...
  event.tutorial.text +=
    '<br>' +
    (this.game().gameConfig === 'cooperative'
      ? '<a>Можно применять либо к блокам игрового поля любой команды, либо к центральным блокам (на этапе финальной интеграции)</a>'
      : '<a>Можно применять как к блокам игрового поля любой команды, так и к центральным блокам</a>');
  ...
  event.init = function () {
    ...
    for (const game of games) {
      ...
      const planes = isCompetitionGame
        ? game.merged
          ? game
              ...
              .decks.table.items()
              .filter(
                (p) => p.anchorGameId === gameId || p.mergedGameId === gameId || p.customClass.includes('central')
              )
      ...
      for (const plane of planes) {
        ...
        eventData.game[game.id()] = { highlight: true };
        eventData.plane[plane.id()] = {
          selectable: true,
          anchorGameId: plane.anchorGameId,
          dicesCount: plane.dicesCount(),
        };
      }
    }
