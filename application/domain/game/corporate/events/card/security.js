(function event() {
  ...
  event.tutorial.text += '<br><a>Можно применить к игровому полю любой команды</a>';
  if (this.game().gameConfig === 'cooperative') {
    event.tutorial.text += '<br>На этапе финальной интеграции также будут убраны фишки с центральных зон интеграции';
  }
  ...
  event.handlers = {
    ...
    TRIGGER: function ({ target: targetGame }) {
      ...
      superGame.logs(
        `Команда <team team="${targetGame.templates.code}">${
          targetGame.title
        }</team> стала целью события <a>${this.getTitle()}</a>.`
      );
      ...
      if (targetGame.merged) {
        const superGameBridges = isCooperativeGame
          ? superGame
              .select({ className: 'Bridge', attr: { sourceGameId: superGame.id() } })
