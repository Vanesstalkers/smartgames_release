(function event() {
  ...
  event.init = function () {
    ...
    const eventData = { dice: {}, player: {}, game: {} };
    ...
    if (Object.keys(eventData.dice).length === 0) {
      activePlayer.notifyUser(`Карта <a>${this.getTitle()}</a> не сработала (нет доступных костей)`);
  ...
  event.handlers['RESET'] = function () {
    ...
    activePlayer.set({ eventData: { dice: null, player: null, game: null } });
    this.destroy();
  };
