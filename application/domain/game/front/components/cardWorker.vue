<template>
  <div v-if="player._id || viewer._id" :id="player._id" :class="[
    ...
    <div v-if="iam" class="tutorial-show end-round-timer"> {{ '15' }}</div>
    <div v-if="!iam" class="tutorial-show card-event" :style="cardEventCustomStyle">
      <div>{{ "2" }}</div>
    </div>
    <div v-if="!iam" class="tutorial-show domino-dice">{{ "3" }}</div>
  </div>
</template>
